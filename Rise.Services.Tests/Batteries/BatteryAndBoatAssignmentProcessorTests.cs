...
namespace Rise.Services.Tests.Batteries
{
    public class BatteryAndBoatAssignmentProcessorTests
    {
        ...
        private Boat _testBoat = new Boat("Test Boat", Boat.BoatStatus.Available);
        private Boat _boat2 = new Boat("Boat2", Boat.BoatStatus.Available);
        private Boat _boat3 = new Boat("Boat3", Boat.BoatStatus.Available);
        private Boat _boat4 = new Boat("Boat4", Boat.BoatStatus.InRepair); 
        private Battery _testBattery = new Battery("Test Battery", Battery.BatteryStatus.Available, new User("auth0UserId", "testuser@example.com"));
        ...
        {   
            ...
            _bookingServiceMock.Verify(
                service =>
                    service.UpdateBookingAsync(
                        booking.Id,
                        It.Is<BookingDto.Mutate>(m => m.Status == BookingDto.BookingStatus.Canceled)
                    ),
                Times.Once
            );
        }
        ...
        public async Task ProcessBatteryAndBoatAssignmentsAsync_ShouldLogError_WhenFetchingBookingsFails()
        {
            ...
            // Assert
            var errorLog = Assert.Single(_loggerProvider.Logs, log => log.Level == LogLevel.Error);
            Assert.Contains("Error occurred while fetching available batteries", errorLog.Message);
            Assert.Same(expectedError, errorLog.Exception);
        }
