...
namespace Rise.Services.Batteries
{
    ...
    /// </summary>
    public class BatteryAndBoatAssignmentProcessor : IBatteryAndBoatAssignmentProcessor
    {
        ...
        public BatteryAndBoatAssignmentProcessor(
        ...
        private async Task<IEnumerable<BatteryDto.BatteryDetail>> GetAvailableBatteriesAsync()
        {
            try
            {
                ...
                var availableOrReserveBatteries = batteries
                    .Where(b =>
                        (
                            b.Status == Battery.BatteryStatus.Available.ToString()
                            || b.Status == Battery.BatteryStatus.Reserve.ToString()
                        )
                    )
        ...
        private async Task AssignBatteriesAndBoatsToBookingsAsync()
        {
            try
            {
                ...
                Console.WriteLine(
                    $"Found {bookings1.Count()} bookings due in 3 days without a battery."
                );
                Console.WriteLine(
                    $"Found {bookings2.Count()} bookings due in 3 days with a battery without a boat."
                );
                ...
                if (bookings1.Any())
                {
                    ...
                    Console.WriteLine(
                        $"Found {availableOrReserveBatteries.Count()} available and reserve batteries."
                    );

                    var reserveBatteries = availableOrReserveBatteries
                        .Where(b => b.Status == Battery.BatteryStatus.Reserve.ToString())
                        .ToList();

                    var availableBatteries = availableOrReserveBatteries
                        .Where(b => b.Status == Battery.BatteryStatus.Available.ToString())
                        .ToList();
        ...
        )
        {
            ...
            if (battery != null)
            {
                ...
                await SendBatteryAssignedEmailAsync(booking, battery);
        ...
        )
        {
            try
            {
                var isReserveBattery = battery.Status == Battery.BatteryStatus.Reserve.ToString();
                _logger.LogInformation(
            ...
            catch (Exception ex)
            {
                _logger.LogError(
                    ex,
                    $"Error assigning battery {battery.Id} to booking {booking.Id} for user with id {booking.UserId}."
                );
            }
        }
        ...
        private async Task CancelBookingAsync(Booking booking, bool noBattery)
        {
            try
            {
                ...
                {
                    RentalDateTime = booking.RentalDateTime,
                    Status = (BookingDto.BookingStatus)Booking.BookingStatus.Canceled,
                    BoatId = booking.Boat?.Id,
                ...
                //TODO: send mail to user that booking is canceled
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, $"Error occurred while canceling booking {booking.Id}.");
            }
        }
        ...
        private async Task<int> GetBoatIdFromBoatWithLongestIdleTime(Booking booking)
        {
            var boatWithLongestIdleTime = await _dbContext
                .Boats.Where(boat => boat.Status == Boat.BoatStatus.Available)
                .GroupJoin(
                    _dbContext.Bookings,
                    boat => boat.Id,
        ...
        public async Task SendBatteryAssignedEmailAsync(
            Booking booking,
            BatteryDto.BatteryDetail battery
        )
        {
