...
public class BoatService(ApplicationDbContext dbContext) : IBoatService
{
    ...
    public async Task<IEnumerable<BoatDto.BoatIndex>?> GetAllBoatsAsync()
    {
        IQueryable<BoatDto.BoatIndex> query = _dbContext
            ...
            .Select(x => new BoatDto.BoatIndex { Id = x.Id, Name = x.Name });
    ...
    {   
        ...
        var newBoat = new Boat(createDto.Name, Boat.BoatStatus.Available);
    ...
    public async Task<BoatDto.BoatIndex?> UpdateBoatStatusAsync(
        int boatId,
        BoatDto.BoatStatus status
    )
    {
        ...
        boat.Status = (Boat.BoatStatus)status;
        await _dbContext.SaveChangesAsync();

        return new BoatDto.BoatIndex { Id = boat.Id, Name = boat.Name };
    }
    ...
    public async Task<int> GetAvailableBoatsCountAsync()
    {
        var availableBoatsCount = await _dbContext
            .Boats.Where(x => !x.IsDeleted && x.Status == (int)Boat.BoatStatus.Available)
            .CountAsync();
