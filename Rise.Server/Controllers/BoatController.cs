...
public class BoatController(IBoatService boatService, ILogger<BatteryController> logger)
    : ControllerBase
{
    ...
    public async Task<ActionResult<BoatDto.BoatIndex>> UpdateBoatStatus(
        int boatId,
        [FromBody] BoatDto.BoatStatus status
    )
    {
        _logger.LogInformation(
        ...
        try
        {
            if (!Enum.IsDefined(typeof(BoatDto.BoatStatus), status))
            {
                _logger.LogError("Invalid boat status \"{Status}\".", status.ToString());
                return BadRequest("Invalid boat status.");
            }

            var updatedBoat = await _boatService.UpdateBoatStatusAsync(boatId, status);
            if (updatedBoat == null)
            {
                _logger.LogWarning("No boat in the system found with id \"{BoatId}\".", boatId);
                return NotFound();
